...
const Contact = () => {
    ...
    const sendEmail = (e) => {
        ...
        emailjs.sendForm('service_0ztwcan', 'template_r074eyr', formRef.current, 'MG2a95a2qxhmS8wfF')
          .then((result) => {
    ...
    <motion.div ref={ref} className='contact' variants={variants} initial="initial" whileInView="animate">
      <motion.div className="textContainer" variants={variants}>
        ...
        <motion.div className="item1" variants={variants}>
            <h2>Mail</h2>
            <span>shivamdotsingh@gmail.com</span>
        </motion.div>
        <motion.div className="item1" variants={variants}>
            <h2>Address</h2>
...
<svg width="450px" height="450px" viewBox="0 0 32.666 32.666">
    ...
    <motion.form ref={formRef} initial={{opacity:0}} whileInView={{opacity:1}} transition={{delay:4,duration: 1}} onSubmit={sendEmail}>
        ...
        {error&&"Error sending E-mail Please try later"}
        {success&&" Sent E-Mail Successfully"}
    </motion.form>
</div>
    </motion.div>
  );
